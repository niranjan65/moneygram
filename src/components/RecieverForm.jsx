...
// // Applies red border when field has an error
...
// /* ─────────────────────────────────────────────────────────
//    DenominationPanel — self-contained, no RHF needed
// ───────────────────────────────────────────────────────── */
...
// /* ─────────────────────────────────────────────────────────
//    GovernmentIdSection — receives RHF helpers as props
// ───────────────────────────────────────────────────────── */
...
//   // Register docFile for validation — not a real input, controlled via setValue
//   register('docFile', {
...
//       idType: 'PASSPORT',   // default selection
//       idNumber: '',
//       docFile: null,
...
//   const rates = useERPNextRates();

// console.log("erpnext rates", rates)

//   const { currencies, rates: allRates, loading: currenciesLoading } = useCurrencies();
//   const [fromCurrency, setFromCurrency] = useState(null);
//   const [toCurrency, setToCurrency] = useState(null);

//   useEffect(() => {
//     if (!currencies.length) return;
//     const defaultFrom = initialData?.senderCurrency ?? 'USD';
//     const defaultTo = initialData?.receiverCurrency ?? 'EUR';
//     setFromCurrency(prev => prev ?? currencies.find(c => c.code === defaultFrom) ?? currencies[0]);
//     setToCurrency(prev => prev ?? currencies.find(c => c.code === defaultTo) ?? currencies[1]);
//   }, [currencies]);

//   // sendAmount is NOT in RHF (it drives denomination panels reactively)
...
//     if (!from || !to || !Object.keys(allRates).length) return;
//     setRateLoading(true);
//     if (from === 'USD') {
//       setLiveRate(allRates[to] ?? null);
...
//           if (data.result === 'success') setLiveRate(data.rates[to] ?? null);
//         } catch {
//           const a = allRates[from], b = allRates[to];
//           if (a && b) setLiveRate(b / a);
//         } finally { setRateLoading(false); }
//       })();
//     }
//   }, [fromCurrency, toCurrency, allRates]);

//   const exchangePreview = useMemo(() => {
//     if (!liveRate || !sendAmount) return null;
//     const raw = sendAmount * liveRate;
//     return {
//       rate: liveRate,
//       rawAmount: raw,
...
//     };
//   }, [liveRate, sendAmount, toCurrency]);
...
//     onSummaryChange({
//       sendAmount,
//       currency: fromCurrency?.code ?? 'USD',
//       exchangeRate: exchangePreview?.rate ?? 0,
//       receiverGets: exchangePreview?.rawAmount ?? 0,
//       receiverCurrency: toCurrency?.code ?? 'EUR',
//     });
//   }, [sendAmount, fromCurrency, toCurrency, exchangePreview]);

//   const senderInfo = useMemo(() =>
//     Object.values(DENOMINATION_DATA).find(d => d.currency === fromCurrency?.code) ?? null,
//     [fromCurrency]);
//   const receiverInfo = DENOMINATION_DATA[watchedCountry] ?? null;

//   // Called only when all RHF validations pass
//   const onSubmit = (data) => {
...
//       ...data,
//       sendAmount,
//       senderCurrency: fromCurrency?.code ?? 'USD',
//       receiverCurrency: toCurrency?.code ?? 'EUR',
//       exchangeRate: exchangePreview?.rate ?? 0,
//       receiverGets: exchangePreview?.rawAmount ?? 0,
...
//       {/* noValidate disables browser native validation so RHF handles everything */}
//       <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8" noValidate>

//         {/* Government ID — passes RHF helpers down */}
...
//                 <Coins size={14} /> You Send <span className="text-red-400">*</span>
//               </label>
//               <input
...
//                 }}
//                 className={`w-full border rounded-lg px-4 py-3 focus:outline-none focus:ring-2 transition text-lg font-black h-14 ${sendAmountError ? 'border-red-400 focus:ring-red-200' : 'border-gray-300'}`}
//                 placeholder="0"
//               />
//               {sendAmountError && <ErrorMsg message={sendAmountError} />}
//               <CurrencyDropdown selected={fromCurrency} onSelect={setFromCurrency} currencies={currencies} loading={currenciesLoading} />
//             </div>
...
//               </label>
//               <input type="text" readOnly placeholder="Converted amount"
//                 value={rateLoading ? 'Loading...' : (exchangePreview?.formatted ?? '')}
//                 className="w-full border border-gray-200 rounded-lg px-4 py-3 bg-gray-100 text-gray-700 font-black text-lg h-14" />
//               <CurrencyDropdown selected={toCurrency} onSelect={setToCurrency} currencies={currencies} loading={currenciesLoading} />
//             </div>
//           </div>
...
//                   1.00 {fromCurrency?.code} = <span style={{ color: PRIMARY }}>{liveRate.toFixed(6)}</span> {toCurrency?.code}
//                 </span>
//                 <p className="text-xs text-gray-400 font-bold">Mid-market rate · Updated just now</p>
//               </div>
//             </div>
//           )}
//         </div>
...
//                 <p className="font-black text-gray-400 text-sm">Select a supported sending currency to enable</p>
//               </div>
//             )}
//             {receiverInfo ? (
//               <DenominationPanel
...
//                 notes={receiverInfo.notes} coins={receiverInfo.coins} targetAmount={exchangePreview?.rawAmount ?? 0}
...
/* ─────────────────────────────────────────────────────────
   buildRows — seed denomination rows from a target amount
───────────────────────────────────────────────────────── */
function buildRows(amount, notes) {
  const rows = [];
  let rem = Math.floor(amount);
...
export const ReceiverForm = ({
  ...
  /* ── FJD is always the sending currency — fixed ── */
  const FJD = { code: 'FJD', symbol: 'FJ$', name: 'Fijian Dollar' };

  /* ── "They Receive" currency — only from ERPNext child table ── */
  const [toCurrency, setToCurrency] = useState(null);
  ...
  /* ── Denomination row refs ── */
  const senderDenomRowsRef   = useRef([]);
  const receiverDenomRowsRef = useRef([]);

  /* ── Auto-select first currency once rates load ── */
  useEffect(() => {
  ...
  /* ── Auto-enable manual entry when today has no ERPNext record ── */
  useEffect(() => {
    if (noDataForToday) setUseManualRate(true);
  }, [noDataForToday]);

  /* ── Effective rate: manual overrides ERPNext when toggled ── */
  const effectiveRate = useMemo(() => {
    if (useManualRate) {
      const m = parseFloat(manualRate);
      return !isNaN(m) && m > 0 ? m : null;
    }
    return toCurrency ? (erpRates[toCurrency.code] ?? null) : null;
  }, [useManualRate, manualRate, toCurrency, erpRates]);
  ...
  /* ── Denomination data ── */
  const senderInfo   = DENOMINATION_DATA['Fiji'] ?? null;   // FJD sender info
  const receiverInfo = DENOMINATION_DATA[watchedCountry] ?? null;
  ...
  const onSubmit = (data) => {
    ...
    if (useManualRate && (!manualRate || parseFloat(manualRate) <= 0)) {
      return; // manual rate input shows its own inline error
    }
    setSendAmountError('');
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-8" noValidate>
        ...
        <div className="flex flex-col gap-4">
          ...
          >
            ...
            {/* THEY RECEIVE — currencies from ERPNext only */}
            <div className="flex flex-col gap-2.5">
              <label style={{ color: PRIMARY }} className="text-xs font-black uppercase tracking-widest flex items-center gap-2">
                <Coins size={14} /> They Receive
              </label>
                  ...
                  >
                    {availableCurrencies.map(c => (
                      <option key={c.code} value={c.code}>
                        {c.code} — Sell: {c.sellingRate.toFixed(2)}  |  Buy: {c.buyingRate?.toFixed(2) ?? '—'}
                      </option>
                    ))}
                  </select>
                  <div style={{ color: PRIMARY }} className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-5 group-hover:scale-110 transition-transform">
          ...
          {/* ── Rate bar + manual override ── */}
          {toCurrency && (
            <div className="flex flex-col gap-3 px-5 py-4 bg-gray-50 rounded-xl border border-gray-200">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex flex-col gap-0.5">
                  <div className="flex items-center gap-2 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                    <RefreshCw size={11} style={{ color: PRIMARY }} />
                    {useManualRate ? 'Manual Override' : 'ERPNext · Selling Rate'}
                  </div>
                  {!useManualRate && effectiveRate && (
                    <span className="font-black text-gray-900 text-sm">
                      1.00 FJD ={' '}
                      <span style={{ color: PRIMARY }}>{effectiveRate.toFixed(2)}</span>{' '}
                      {toCurrency.code}
                    </span>
                  )}
                  {rateDate && !useManualRate && (
                    <p className="text-[11px] text-gray-400 font-bold">
                      {noDataForToday
                        ? `⚠ No record for today — using ${rateDate}`
                        : `Rate date: ${rateDate}`}
                    </p>
                  )}
                </div>
                ...
                <button
                  type="button"
                  onClick={() => { setUseManualRate(v => !v); setManualRate(''); }}
                  style={useManualRate
                ...
                >
                  ...
                  {useManualRate ? 'Use ERPNext Rate' : 'Override Rate Manually'}
                </button>
              </div>

              {/* Manual rate input — shown when toggled or no data for today */}
              {useManualRate && (
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                    Selling Rate — 1 FJD = ? {toCurrency.code} <span className="text-red-400">*</span>
                  </label>
                  <div
                    style={{
                      borderColor: !manualRate || parseFloat(manualRate) <= 0
                        ? '#f87171'
                        : `${PRIMARY}66`,
                    }}
                    className="flex items-center rounded-xl border bg-white overflow-hidden h-12"
                  >
                    <span className="pl-4 text-gray-400 font-black text-sm shrink-0">1 FJD =</span>
                    <input
                      type="number" min="0.0001" step="0.0001"
                      placeholder="e.g. 10.56"
                      value={manualRate}
                      onChange={e => setManualRate(e.target.value)}
                      className="h-full px-3 flex-1 text-sm font-black focus:outline-none bg-transparent"
                      autoFocus
                    />
                    <span className="pr-4 text-gray-500 font-black text-sm shrink-0">
                      {toCurrency.code}
                    </span>
                  </div>
                  {(!manualRate || parseFloat(manualRate) <= 0) && (
                    <ErrorMsg message="Please enter a valid selling rate" />
                  )}
                </div>
              )}
            </div>
